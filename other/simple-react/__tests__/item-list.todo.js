...
test('returns no items when no items', ()=>{
    const container = document.createElement('div');
    ReactDOM.render(<ItemList items={[]}/>, container);
    expect(container.textContent).toMatch('no items');
});

test('returns items when there are items', ()=>{
  const container = document.createElement('div');
  ReactDOM.render(<ItemList items={['apple', 'orange', 'banana']} />, container);
  //expect(container.len).toHaveLength(3);
});
